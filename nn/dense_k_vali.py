...
def preprocess(filename, kfold):
    ...
    x_train_list = np.zeros((kfold, fold_size, 310))
    y_train_list = np.zeros((kfold, fold_size, 1))
    for i in range(kfold):
    	x_train_list[i] = x_train_new[i*fold_size:(i+1)*fold_size]
    	y_train_list[i] = y_train_new[i*fold_size:(i+1)*fold_size]
    print 'train'
...
def train_generator(filename, kfold,batch_size):
    x_train_list, y_train_list, x_test, y_test = preprocess(filename,kfold)
    y_train_list_new = np.zeros((y_train_list.shape[0], y_train_list.shape[1], 3))
    for i in range(kfold):
    	y_train_list_new[i] = np_utils.to_categorical(y_train_list[i],3)
    i = 0

    while 1:
    	if (i==kfold):
    	    i=0
    	tmp_x = x_train_list[i]
    	tmp_y = y_train_list_new[i]
    	for j in range(kfold-2):
    	    tmp_x = np.vstack((tmp_x, x_train_list[(j+i+1)%kfold]))
    	    tmp_y = np.vstack((tmp_y, y_train_list_new[(j+i+1)%kfold]))
    	#print i, (i+kfold-2)%kfold
        times = tmp_x.shape[0]/batch_size
        print times
        for j in range(times):
            yield tmp_x[j*batch_size:(j+1)*batch_size], tmp_y[j*batch_size:(j+1)*batch_size]
    	i+=1
def valid_generator(filename, kfold):
    x_train_list, y_train_list, x_test, y_test = preprocess(filename,kfold)
    y_train_list_new = np.zeros((y_train_list.shape[0], y_train_list.shape[1], 3))
    for i in range(kfold):
    	y_train_list_new[i] = np_utils.to_categorical(y_train_list[i],3)
    ...
    while 1:
    	...
    	yield x_train_list[(i+kfold-1)%kfold], y_train_list_new[(i+kfold-1)%kfold]
        print 'validation on', i
    	i+=1
def train_and_evaluate(filename, model_name,batch_size = 320,epochs = 6,kfold = 10,steps_per_epoch = 800):
    ...
    model.fit_generator(train_generator(filename, kfold,batch_size), steps_per_epoch = steps_per_epoch, epochs=epochs, validation_data = valid_generator(filename, kfold), validation_steps = 10)
    #model.fit(x_train, y_train, epochs = epochs, batch_size = batch_size, shuffle = False, validation_split = 0.4)
    x_train_list, y_train_list, x_test, y_test = preprocess(filename,kfold)
...
def main():
    filename = '../EEG.mat'
    train_and_evaluate(filename =filename, model_name = '6_14_1.h5', epochs = 12,batch_size =140,steps_per_epoch = 800 )
